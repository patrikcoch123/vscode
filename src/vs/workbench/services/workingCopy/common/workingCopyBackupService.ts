...
class WorkingCopyBackupServiceImpl extends Disposable implements IWorkingCopyBackupService {
	...
	private async doInitialize(): Promise<WorkingCopyBackupsModel> {
		...
		for (const backupResource of this.model.get()) {
			...
			try {
				const identifier = await this.resolveIdentifier(this.model, backupResource);
				if (!identifier) {
	...
	async getBackups(): Promise<IWorkingCopyIdentifier[]> {
		...
		const backups = await Promise.all(model.get().map(backupResource => this.resolveIdentifier(model, backupResource)));

		return coalesce(backups);
	}

	private async resolveIdentifier(model: WorkingCopyBackupsModel, backupResource: URI): Promise<IWorkingCopyIdentifier | undefined> {
		...
		let meta: IWorkingCopyBackupMeta | undefined = undefined;
		let typeId: string | undefined = undefined;
		if (metaPreamble) {
			try {
				const parsedPreambleMeta = this.parsePreambleMeta(metaPreamble);
				meta = parsedPreambleMeta.meta;
				typeId = parsedPreambleMeta.typeId;
			} catch (error) {
	...
	private parsePreambleMeta<T extends IWorkingCopyBackupMeta>(preambleMetaRaw: string): { typeId: string | undefined, meta: T | undefined } {
		let typeId: string | undefined = undefined;
