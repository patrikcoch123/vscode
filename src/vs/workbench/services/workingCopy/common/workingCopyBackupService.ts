...
class WorkingCopyBackupServiceImpl extends Disposable implements IWorkingCopyBackupService {
	...
	private async doInitialize(): Promise<WorkingCopyBackupsModel> {
		...
		// Create backup model
		const model = this.model = await WorkingCopyBackupsModel.create(this.backupWorkspaceHome, this.fileService);
		...
		for (const backupResource of this.model.get()) {
			...
			try {
				const identifier = await this.resolveIdentifier(backupResource, model);
				if (!identifier) {
				...
				if (!isEqual(expectedBackupResource, backupResource)) {
					...
					model.move(backupResource, expectedBackupResource);
				}
			} catch (error) {
